...
| [TIMDR-AI-Core](https://github.com/jbackk-lang/TIMDR-AI-Core) | Rdzeń protokołu TIMDR i silnik audytu twierdzeń `claim_audit.py` — karty dowodowe przeliczają liczby z README z surowych danych, sprawdzają kotwice pre-rejestracji i sformułowania; użyty do audytu MAST, Modal-Formalism, Aviation, Bio-Signals i MAGE (znalezione i poprawione realne błędy). Linie eksperymentów aktywacji zamknięte bez przyrostu nad baseline'em |
| [TIMeDR-MUZ](https://github.com/jbackk-lang/TIMeDR-MUZ) | Lokalny zarządca budżetu osobistego na regułach TIMDR (sygnały, fazy, polityka budżetowa, pisma), realne wskaźniki CPI z GUS; użytkownik decyduje tylko przy bramce zatwierdzeń, mini-AI działa wyłącznie w trybie cienia. Bez technologii Meta, w pełni lokalny. Nie jest doradztwem finansowym i nie wykonuje przelewów |
| [TIMDR-Cyclone-RI](https://github.com/jbackk-lang/TIMDR-Cyclone-RI) | Cztery cechy burzy poprzedzające wyrównanie pionowe i gwałtowną intensyfikację cyklonu tropikalnego (Alvey i in. 2026, JGR-Atmospheres) — etykietowanie intensyfikacji zwalidowane na realnych danych, operatory przechylenia i środowiska tylko na kontrolach syntetycznych (brak dostępu do TC-RADAR), opisane wprost w repo |
---
### 🧮 Formalizacje TIMDR
...
| [TIMDR-Sygnalizacja](https://github.com/jbackk-lang/TIMDR-Sygnalizacja) | Formalizm sygnałowy TIMDR (anomalia/defekt/rezonans/skręt) na trzech historycznych metodach sygnalizacji — kod Morse'a dźwiękowy, Morse świetlny (lampa Aldisa), semafor flagowy; dekodery + 4 detektory + kontrole pozytywne/negatywne z testem Manna-Whitneya, panel web z obsługą mikrofonu/kamery. Zwalidowane na sygnałach syntetycznych; funkcje na żywym sprzęcie (mikrofon/kamera) niezweryfikowane w izolowanym środowisku budowy |
---
### 🌀 Modele koncepcyjne
...
---
*Podział sporządzony na podstawie przeglądu kodu (nie tylko README) w sierpniu 2026, zaktualizowany we wrześniu 2026 o kategorię „Formalizacje TIMDR" i o sekcję powiązań kodu (10 września 2026 — **zamiana mechanizmu współdzielenia kodu**: stary wzorzec sibling-import przez `sys.path.insert()`, działający tylko przy konkretnym układzie folderów na jednym dysku, zastąpiono wendoringiem, czyli kopią 1:1 z jawnym nagłówkiem „ZWENDOROWANE" w pliku źródłowym, po uniezależnieniu repo od sibling-importu), o TIMDR-Solar-PV i TIMDR-Mold-Risk (14 września 2026 — dwie nowe domeny, fizycznie/naukowo ugruntowane modele PVWatts i VTT Mold Growth, oba jeszcze bez walidacji na realnych danych, patrz opisy w tabeli), o dwa wcześniej pominięte repo, TIMDR-Tornado-NEXRAD i TIMDR-Sygnalizacja (16 września 2026 — istniały w profilu, ale nie były wpisane do tego indeksu; znalezione przy audycie kompletności), oraz o pełny audyt tej zamiany na wszystkich 13 wpisach tabeli powyżej (19 września 2026 — potwierdzono brak żywych importów międzyrepozytoryjnych na każdym z nich, każde repo działa samodzielnie po sklonowaniu tylko siebie). Kategoria „narzędzia inżynierskie" oznacza, że w repozytorium znajduje się działający kod przetwarzający realne dane wejściowe — nie jest to gwarancja bezbłędności, tylko potwierdzenie, że narzędzie robi to, co deklaruje. Kategoria „Formalizacje TIMDR" oznacza działający, przetestowany kod bez tego wymogu realnych danych. Zaktualizowano 2026-09-20: FAI i FUNDAMENTAL-AI-MODEL-WERSJA-PRO-main scalone jako podkatalogi (`FAI`, `FUNDAMENTAL-AI-MODEL-WERSJA-PRO`) do `TIMDR-Concept-Archive` przez `git subtree`, oryginały usunięte z GitHuba (łącznie 18 pozycji w archiwum). Zaktualizowano 2026-09-26: dodano trzy nowe repozytoria — TIMDR-AI-Core, TIMeDR-MUZ i TIMDR-Cyclone-RI; testy diagnostyczne operatorów TIMDR na pięciu stanowiskach (CWRU, SEU, budynek LANL, prądy silnika Paderborn, wideo UCSD Ped2) nie wykazały przewagi nad klasycznymi metodami — TIMDR opisuje się odtąd jako ramę opisu i protokół badania, nie detektor (tabela w README GIA-TIMDR).*
